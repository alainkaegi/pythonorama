...
```python
plus_four = create_adder(4)
```

The expression `create_adder(4)` returns a function that takes a single argument (`x`) and returns the value of the expression `x + 4`. The name `plus_four` is assigned to this function. Therefore `plus_four(3)` returns `7`.
